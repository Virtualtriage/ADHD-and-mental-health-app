...
const STRIPE_KEY = 'pk_live_51T6jsBCvFmGnyQXn4NfGkZtYokU0urTRv86ISK4zLkhBoGlCzaEhzpOtkQXE8oktxzu6kPruKbDDGLHOrmfp900W00SMXZb7Cx';
...
const CheckoutForm = ({ appointmentData, onSuccess, onClose, StripeHooks }) => {
  ...
  const validatePromoCode = async () => {
    ...
    const authToken = appointmentData.token || localStorage.getItem("authToken") || sessionStorage.getItem("authToken");
    if (!authToken) {
      setPromoError("Please log in to use promo codes.");
  ...
  const handleSubmit = async (event) => {
    ...
    // Check authentication before submitting payment
    const authToken = appointmentData.token || localStorage.getItem("authToken") || sessionStorage.getItem("authToken");
    if (!authToken) {
